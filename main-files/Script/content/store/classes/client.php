...
class Client {
	...
	function add_client($full_name, $client_code, $business_title, $mobile, $phone, $address, $city, $state, $zipcode, $country, $email, $price_level, $notes) {
		global $db;
		$query = "SELECT * from clients WHERE full_name='".$full_name."' AND store_id='".$_SESSION['warehouse_id']."'";
		$result = $db->query($query) or die($db->error);
		$num_rows = $result->num_rows;
		
		if($num_rows > 0) { 
			return '<div class="alert alert-danger"><i class="fa fa-exclamation-triangle"></i> A customer with same name already exists.</div>';
		} else { 
			$query = "INSERT into clients(client_id, client_code, full_name, business_title, mobile, phone, address, city, state, zipcode, country, email, price_level, status, store_id)
				VALUES(NULL, '".$client_code."', '".$full_name."', '".$business_title."', '".$mobile."', '".$phone."', '".$address."', '".$city."', '".$state."', '".$zipcode."', '".$country."', '".$email."', '".$price_level."', '1', '0')
			";
			$result = $db->query($query) or die($db->error);
			$_SESSION['cn_id'] = $db->insert_id;
			return '<div class="alert alert-success"><i class="fa fa-check-circle"></i> Customer added successfuly.</div>';
		}
	...
	function disable_customer($customer_id) {
		global $db;
		
		$query = 'UPDATE clients SET status = "0" WHERE client_id = "'.$customer_id.'" ';
		$result = $db->query($query) or die($db->error);
		return '<i class="fa fa-check-circle"></i> Customer Desactivated Successfuly!';
	}//update user level ends here.	
	function enable_customer($customer_id) {
		global $db;
		
		$query = 'UPDATE clients SET status = "1" WHERE client_id = "'.$customer_id.'" ';
		$result = $db->query($query) or die($db->error);
		return '<i class="fa fa-check-circle"></i> Customer Activated Successfuly!';
	}//update user level ends here.	
	
	function update_client($client_id, $full_name, $business_title, $mobile, $phone, $address, $city, $state, $zipcode, $country, $email, $status) { 
		global $db;
		...
		$result = $db->query($query) or die($db->error);
		return '<div class="alert alert-success"><i class="fa fa-check-circle"></i> Customer updated Successfuly!</div>';
	}//update user level ends here.	
	...
	function list_clients() {
		...
		while($row = $result->fetch_array()) { 
			...
			$content .= '</td>';
				if(partial_access('admin')) {
					$content .= '<td><form method="post" name="edit" action="newcustomer.php">';
					$content .= '<input type="hidden" name="edit_customer" value="'.$client_id.'">';
					$content .= '<button type="submit" class="btn btn-success" value="Edit" title="Edit Customer"><i class="fa fa-cog" aria-hidden="true"></i></button>';
					$content .= '</form>';
					$content .= '</td><td>';
					$content .= '<form method="post" name="enable" onsubmit="return confirm_delete();" action="customers.php">';
					$content .= '<input type="hidden" name="enable_customer" value="'.$client_id.'">';
					$content .= '<button type="submit" class="btn btn-info" value="Enable" title="Activate Customer"><i class="fa fa-unlock" aria-hidden="true"></i></button>';
					$content .= '</form>';
					$content .= '</td>';
					}
			} elseif ($status=='1') {
			...
			$content .= '</td>';
				if(partial_access('admin')) {
					...
					$content .= '</form>';
					$content .= '</td><td>';
					$content .= '<form method="post" name="disable" onsubmit="return confirm_delete();" action="customers.php">';
					$content .= '<input type="hidden" name="disable_customer" value="'.$client_id.'" >';
					$content .= '<button type="submit" class="btn btn-danger" value="Disable" title="Deactivate Customer"><i class="fa fa-lock" aria-hidden="true"></i></button>';
					$content .= '</form>';
					$content .= '</td>';
					}
			}
