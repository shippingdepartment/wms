...
class Product {
	...
	function add_product($product_manual_id, $product_name, $supplier_id, $product_unit, $category_id, $tax_id, $product_cost, $product_selling_price, $alert_units, $long_pr, $larg, $haut, $poids) { 
		...
		if($num_rows > 0) { 
			...
			$query = "INSERT into products (product_id, product_manual_id, product_name, supplier_id, product_unit, category_id, tax_id, alert_units, warehouse_id) VALUES(NULL, '".$product_manual_id."', '".$product_name."', '".$supplier_id."', '".$product_unit."', '".$category_id."', '".$tax_id."', '".$alert_units."', '".$_SESSION['warehouse_id']."')";
			$result = $db->query($query) or die($db->error);
	...
	function product_mvt($product_id) { 
		...
		$result = $db->query($query) or die($db->error);
			...
			while($row = $result->fetch_array()) { 
				...
				$content .= $product_name;
				$content .= '</td><td>';
				if($transfer_id !=0) { $operation = " Goods Transferred "; }
				elseif($delivery_id !=0) { $operation = " Goods Sold "; }
				elseif($return_id !=0) { $operation = " Goods Returned "; }
				elseif($order_id !=0) { $operation = " Goods Ordered "; }
				else {  $operation = " Stock Add ";   }
				$content .= $operation;
				$content .= '</td >';
				...
				$content .= '<td>';
				if($transfer_id !=0) { $reference = " Transfer N#: ".$transfer_id; }
				elseif($delivery_id !=0) { $reference = " Delivery N#: ".$delivery_id; }
				elseif($return_id !=0) { $reference = " Goods Returned Return N#: ".$return_id; }
				elseif($order_id !=0) { $reference = " Order N#: ".$order_id; }
				else {  $reference = " Stock Add Lot# ".$lot;   }
				$content .= $reference;
				$content .= '</td></tr>';
