<?php
	...
	if (isset($_POST['nb'])){
			$nb=$_POST['nb'];
			$datetime = date('Y-m-d');
			$return_id = $delivery->add_return_delivery($did, $datetime);
			for ($i = 0; $i < $nb; $i++) {
				$pid = $_POST['pid'][$i];
				$qty = $_POST['qty'][$i];
				$pname = $_POST['pname'][$i];
				...
				//Add Inventory
				$delivery->add_inventory_return($datetime, $qtyapp, 0, $pid, $_SESSION['warehouse_id'], $return_id);
				}
				$note_title = 'New Return Received';
			    $note_details = 'New Return #: '.$return_id.'( from Delivery N#: '.$did.')';
	...
	$page_title = 'Stock Return from Delivery N#: '.$did; //You can edit this to change your page title.
							...
							<div class="panel panel-white alert alert-default" >
								...
								<form class="form-horizontal" action="newreturn.php" id="wizardForm" name="level" method="post">
									...
									<div class="form-group">
										...
										<div class="col-sm-2">
											<input type="text" class="form-control" name="datetime"   value="<?php echo $delivery->get_delivery_info($did,'datetime'); ?>" Disabled />
									...
									<div class="form-group">
										...
										<div class="col-sm-2">
											<input type="text" class="form-control" name="dateissued"   value="<?php echo $delivery->get_delivery_info($did,'dateissued'); ?>" Disabled />
													
										</div>
									</div>
									<?php $client_id = $delivery->get_delivery_info($did,'client_id'); 
									 $client = new Client;
									...
									<div class="form-group">
										...
										<div class="col-sm-8">
											<input type="text" class="form-control" name="client"   value="<?php echo $client_name;?>" Disabled />
									...
									<form action="returndelivery.php?did=<?php echo $did; ?>" class="form-horizontal" method='POST' name='testform'>
										...
										<table cellpadding="0" cellspacing="0" border="0" class="table-responsive table-hover table display table-bordered" id="example3" width="100%">
											<thead >
												<tr>
													...
													<!--<th width="10%"><font color="#fff">Qty OK </th>-->
												</tr>
											</thead>
											<tbody>
												<?php $delivery->return_delivery_details($did); ?>
											</tbody>
