...
class WarehouseAccess {
	...
	function add_warehouse_access($user_id, $warehouse_id, $access_to) { 
		global $db;
			...
			if($rows > 0) { 
				...
				foreach($access_to as $access) { 
					if($access == 'products') { 
						...
						$orders = 1;
					} else if($access == 'suppliers') { 
						$suppliers = 1;
					} else if($access == 'stock') { 
						$stock = 1;
					} else if($access == 'receptions') { 
						$receptions = 1;
				...
				$query = "INSERT into warehouse_access(user_id, warehouse_id, products, transfers, deliveries, clients, reports, orders, suppliers, stock, receptions, returns) VALUES('".$user_id."', '".$warehouse_id."', '".$products."', '".$transfers."', '".$deliveries."', '".$clients."', '".$reports."', '".$orders."', '".$suppliers."', '".$stock."', '".$receptions."', '".$returns."')";
				$result = $db->query($query) or die($db->error);
				return '<div class="alert alert-success">Access granted successfuly.</div>';
			}
		} else { 
			return '<div class="alert alert-danger">You have not the Permission to grant Access !!</div>';
		}
	...
	function delete_access($access_id) {
		...
		if($_SESSION['user_type'] == 'admin' && $access_id != '') { 
			$query = "DELETE from warehouse_access WHERE access_id='".$access_id."'";
			$result = $db->query($query) or die($db->error);
			return '<div class="alert alert-success"><i class="fa fa-check-circle"></i> User access deleted successfuly!</diV>';
		}//if admin
	}//delete acces function ends here.
	
	function have_warehouse_access() {
		global $db;
		$query = "SELECT * from warehouse_access WHERE user_id='".$_SESSION['user_id']."' AND warehouse_id='".$_SESSION['warehouse_id']."'"; 
		$result = $db->query($query) or die($db->error);
