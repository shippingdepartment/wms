	...
	function get_user_accesses() {
		...
		if($user_type == 'admin') {
			...
			while($row = $result->fetch_array()) { 
				...
				$content .= '</td><td>';
				if($clients==1) { $cl="Yes"; } else {$cl="No"; }
				$content .= $cl ;
				$content .= '</td><td>';
				if($products==1) { $pr="Yes"; } else {$pr="No"; }
				$content .= $pr;
				$content .= '</td><td>';
				if($transfers==1) { $tr="Yes"; } else {$tr="No"; }
				$content .= $tr;
				$content .= '</td><td>';
				if($orders==1) { $or="Yes"; } else {$or="No"; }
				$content .= $or;
				$content .= '</td><td>';
				if($deliveries==1) { $de="Yes"; } else {$de="No"; }
				$content .= $de;
				$content .= '</td><td>';
				if($suppliers==1) { $sp="Yes"; } else {$sp="No"; }
				$content .= $sp;
				$content .= '</td><td>';
				if($stock==1) { $sk="Yes"; } else {$sk="No"; }
				$content .= $sk;
				$content .= '</td><td>';
				if($receptions==1) { $rc="Yes"; } else {$rc="No"; }
				$content .= $rc;
				$content .= '</td><td>';
				if($returns==1) { $rn="Yes"; } else {$rn="No"; }
				$content .= $rn;
				$content .= '</td><td>';
				if($reports==1) { $rp="Yes"; } else {$rp="No"; }
				$content .= $rp;
				$content .= '</td>';
