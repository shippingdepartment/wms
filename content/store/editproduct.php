...
if (isset($_POST['edit_product'])) {
	...
	$photo = $product->get_product_info($p, 'photo');
	$photos = $product->get_product_info($p, 'photo');
}
		...
		if (isset($_FILES['productPhoto'])) {
			if (($_FILES['productPhoto']['error'] == 4)) {
				...
				if (in_array($imageFileType, $extensions_arr)) {
					if (move_uploaded_file($_FILES['productPhoto']['tmp_name'], $target_dir . $photo)) {
					}
				}
				...
				<div class="panel panel-white alert alert-default">
					<div class="panel-heading clearfix">
						<div class="panel-body">
							<form id="wizardForm" class="form-horizontal " style="font-size:14px;color:#0d47a1" action="<?php $_SERVER['PHP_SELF']; ?>" method="POST" enctype='multipart/form-data'>
								<label for="input-Default" class="col-sm-2 control-label">Product Code / Barcode:</label>
								<div class="col-sm-10">
									<div class="form-group">
										<input type="hidden" name="prodid" value="<?php if (isset($p) && ($p != '')) {
																						echo $p;
																					} ?>" />
								...
								<div class="col-sm-10">
									<div class="form-group">
										<select id="unit" name="unit" class="form-control" style="width:100%">
											...
											<option value='unit' <?php if ($unit == 'unit') {
																		echo 'selected';
																	} ?>>Unit</option>
											<option value='pounds' <?php if ($unit == 'pounds') {
																		echo 'selected';
																	} ?>>Pounds</option>
											<option value='ounce' <?php if ($unit == 'ounce') {
																		echo 'selected';
																	} ?>>Ounce</option>
											<option value='box' <?php if ($unit == 'box') {
																	echo 'selected';
																} ?>>Box</option>
											<option value='cm' <?php if ($unit == 'cm') {
																	echo 'selected';
																} ?>>Cm</option>
								...
								<label for="input-Default" class="col-sm-2 control-label">productPhoto</label>
								<div class="col-sm-10">
									<div class="form-group">
										<input type="file" id="productPhoto" name="productPhoto" class="form-control" placeholder="">
								...
								<!-- <div class="form-group">
									...
									<div class="col-sm-10">
										<div style="margin-bottom:15px;" class="input-group"> -->
								<input type="hidden" id="weight" name="weight" class="form-control" placeholder="Weight" value=" echo $product->poids; ?>" onkeypress="return isNumberKey(event)" required>
								<!-- </div>
									</div>
								</div> -->
								...
								<div class="form-group">
									...
									<div class="col-sm-10">
										<div style="margin-bottom:15px;" class="input-group">
											<input type="text" id="pounds" name="pounds" class="form-control" placeholder="Weight" value="<?php echo $product->pounds; ?>" onkeypress="return isNumberKey(event)" required><span class="input-group-addon">lb</span>
										</div>
									</div>
								</div>
